...
#사용자로 부터 숫자 N를 입력받아, "long"을 N/4번 반복하고 마지막에 "int"를 추가하여 출력한다
N = int(input())
print(" ".join(["long"] * (N // 4) + ["int"]))


노트
실제로 C++에서 각 정수 자료형이 저장할 수 있는 수의 크기는 환경에 따라 달라질 수 있다. 덧붙여, 
실제로 문제 내용과 같이 long long long int와 같은 자료형을 사용한 코드를 GCC의 C++ 컴파일러를 사용해 
